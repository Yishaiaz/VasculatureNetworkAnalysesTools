...
def get_all_subgraph_features(_g: Graph, e_features: list[str], v_features: list[str], sg_type: str, _d: int, add_basics: bool=True) \
        -> pd.DataFrame:
    """
...
def add_full_graph_properties(_g: Graph, cancer_array_path: str, region_name: str):
  ...
  #  cancer_coords = fe.get_cancer_coords(cancer_array_path, region_name) TODO return

    # adding new features to graph
 #   add_vertex_distance_to_closest_target_object(_g, cancer_coords) TODO return
...
def add_features_to_existing(features_file, new_feature):
    df = pd.read_pickle(features_file)
    all_features_df = pd.concat([df, new_feature], axis=1)
    all_features_df.to_pickle(features_file)


def main(
        graph_path="/Users/leahbiram/Desktop/vasculature_data/firstGBMscanGraph.gt",
        graph_name: str = "firstGBMscanGraph",
    ...
    edge_features = ["radii", "length", "artery_binary", "artery_raw", "prolif", "x_direction", "y_direction", "z_direction"]
    ...
    #adding specific features not from subgrphs TODO return
...
#                                                   if not fe.vertex_disconnected(g, cv)]
    ...
    os.makedirs('../ExtractedFeatureFiles', exist_ok=True)
    all_features_df.to_pickle("../ExtractedFeatureFiles/" + graph_name + ".csv")
